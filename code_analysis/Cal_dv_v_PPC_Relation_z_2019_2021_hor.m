...
% velocity changes (dv/v) from horizontal component. 
%
% Physical basis:
...
%
% Output: Relation_PPC_z_Hor.mat containing optimization results
%
% Author: FENG CHEN
...
new_fitted_dv_v_hor=mean(dv_mean(:,[2,3,5,6]),2);  % Average of horizontal components
nan_data=isnan(new_fitted_dv_v_hor);
new_dv_v_clean_hor=new_fitted_dv_v_hor(~nan_data);
new_dv_v_mean_hor=mean(new_dv_v_clean_hor);

%% calculated fitted z for hor
Rain_data(1:2)=[];
Rain_data(323:end)=[];
...
for day_idx=1:n-length(Tdata)+1
    for jj=1:1000
        ...
        P_clean=P(~nan_data);
        P_mean=mean(P);
        cov_matrix = cov(new_dv_v_clean_hor, P_clean);
        cov_dv_v_P = cov_matrix(1, 2);
        var_P = var(P_clean);
        % Formula: dv/v_syn = mean(dv/v) + (cov(dv/v,P)/var(P)) × (P - mean(P))
        tmp_dv_v_syn(:, jj, day_idx) =new_dv_v_mean_hor+ cov_dv_v_P / var_P * (P_clean - P_mean);
        delta_sigma_hor(jj, day_idx) = sum((tmp_dv_v_syn(:, jj, day_idx) - new_dv_v_clean_hor).^2);
    end
    % Display progress for current starting date
    disp(['delta_sigma(:, ', num2str(day_idx), ') = ', num2str(delta_sigma_hor(:, day_idx)')]); 
end
save Relation_PPC_z_Hor.mat
