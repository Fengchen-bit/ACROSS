%% Seismic Velocity Change Analysis Using Stretching Method (Coda Wave Interferometry) in Tt component
...
% - Tt_data.mat: Daily seismogram data for radial component
% - Coda_Tt.mat: Coda wave end times for each station
%
% Output: dv_v_Tt.mat containing velocity changes and correlations
%
...
seisdix=size(daily_data,3);
dv_v_Tt=nan(dayidx,seisdix);  % 322*14
CC_Tt=nan(dayidx,seisdix);  % 322*14
for seis_ch=1:seisdix %14
    ...
    for day_num=1:length(t)
        ...
        if ~any(isnan(currentTF))
            ...
            dv_v_Tt(day_num,seis_ch) = bestStretchRatio*100;
            CC_Tt(day_num,seis_ch) = maxCorrelation;
        end
    end
    fprintf('%05.0f of %05.0f Done. %05.2f [sec]\n',seis_ch,seisdix,toc(st))
end

save dv_v_Tt cut_daily_data cut_ref_data t dv_v_Tt CC_Tt
