%% Seismic Velocity Change Analysis Using Stretching Method (Coda Wave Interferometry) in Ut component
...
% - Ut_data.mat: Daily seismogram data for Ut component
% - Coda_Ut.mat: Coda wave end times for each station
%
...
seisdix=size(daily_data,3);
dv_v_Ut=nan(dayidx,seisdix);  % 322*14
CC_Ut=nan(dayidx,seisdix);  % 322*14
for seis_ch=1:seisdix %14
    ...
    for day_num=1:length(t)
        ...
        if ~any(isnan(currentTF))
            ...
            dv_v_Ut(day_num,seis_ch) = bestStretchRatio*100;
            CC_Ut(day_num,seis_ch) = maxCorrelation;
        end
    end
    fprintf('%05.0f of %05.0f Done. %05.2f [sec]\n',seis_ch,seisdix,toc(st))
end

save dv_v_Ut cut_daily_data cut_ref_data t dv_v_Ut CC_Ut
