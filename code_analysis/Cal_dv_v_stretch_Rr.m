%% Seismic Velocity Change Analysis Using Stretching Method (Coda Wave Interferometry) in Rr component
...
% - Rr_data.mat: Daily seismogram data for Rr component
% - Coda_Rr.mat: Coda wave end times for each station
%
% Output: dv_v_Rr.mat containing velocity changes and correlations
%
...
load('Coda_Rr.mat');
fsamp=100; % Sampling frequency (Hz)
coda_start=5; % Coda wave start time 
coda_end=Coda_end; % Rr
...
seisdix=size(daily_data,3);
dv_v_Rr=nan(dayidx,seisdix);  % 322*14
CC_Rr=nan(dayidx,seisdix);  % 322*14
for seis_ch=1:seisdix %14
    ...
    for day_num=1:length(t)
        ...
        if ~any(isnan(currentTF))
            ...
            dv_v_Rr(day_num,seis_ch) = bestStretchRatio*100;
            CC_Rr(day_num,seis_ch) = maxCorrelation;
        end
    end
    fprintf('%05.0f of %05.0f Done. %05.2f [sec]\n',seis_ch,seisdix,toc(st))
end

save dv_v_Rr cut_daily_data cut_ref_data t dv_v_Rr CC_Rr
