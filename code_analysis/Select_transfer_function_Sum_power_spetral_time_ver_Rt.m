...
%
% Output: Rt_data.mat containing reference and daily transfer functions
%
% Author: FENG CHEN
...
for ii=1:length(tfdata)
    Rt_data(:,ii,:)=tfdata{ii}(:,:,5); % 5000*3864*14 Rt  % - Components Ur-1 Rr2- Tr-3 Ut-4 Ur-5 Tt-6
...
while current_day <= end_day
    ...
    for hour = 1:12  % Check 12 measurement times per day (every 2 hours: 1, 3, 5, ..., 23)
        ...
        if ~isempty(idx)  
            ...
            FFt_data=fft(squeeze(Rt_data(:,expected_idx,:))); % 5000*14 fft
...
for seisidx=1:14
    tmp_valid_idx = setdiff(1:size(Rt_data, 2), seis_nan_idx{seisidx}); 
    ...
    ref_data(:,seisidx)=mean(squeeze(Rt_data(:,valid_idx{seisidx},seisidx)),2);
...
while current_day <= end_day
    ...
    for seisidx=1:14
        ...
        if ~isempty(exeidx) & length(exeidx)>=6
            daily_data(:,day_idx,seisidx)=mean(squeeze(Rt_data(:,valid_idx{seisidx}(exeidx),seisidx)),2);
        end
    end
    day_idx=day_idx+1;
    current_day = current_day + days(1);
...
save('Rt_data','t','tt','ref_data','daily_data');